...
![OpenChainClaw 信任闭环](docs/assets/readme-trust-loop.svg)
...
- [项目蓝图](docs/PROJECT_BLUEPRINT.md)：产品假设、能力分层、非目标和成功信号。
- [MVP 范围](docs/MVP_SCOPE.md)：必须交付、应该交付、后续能力和暂不进入 MVP 的边界。
- [用户流程](docs/USER_FLOWS.md)：任务创建、风险审批、文件回滚、证明验证、首次配置和手机端控制流程。
- [决策记录](docs/DECISIONS.md)：需要长期保持一致的产品和工程决策。
- [安全模型](docs/SECURITY_MODEL.md)：信任边界、敏感数据类型、风险行为和证明规则。
- [Analytics](docs/analytics.md)：可选 PostHog 事件目录、验证命令和隐私规则。
- [Mobile IM Control Safety Model](docs/mobile-im-control.md)：未来即时通讯适配器的安全约束。

本地私有规划、调研材料、凭据、审计导出和未公开商业信息会刻意排除在 git 之外。
...
4. 文件修改前创建快照，并展示差异；
5. 支持的文件修改可以回滚；
6. 本地审计哈希和证明元数据让记录可检查。

证明层刻意保持克制。它的目标是验证本地审计记录，而不是把私有任务数据复制到 analytics、公开文档或外部证明系统里。未来证明集成应优先考虑策略证明和选择性披露，而不是公开身份图谱或公开任务历史。
...
- 更丰富的证明后端和验证查看体验；
- 能证明策略合规但不暴露原始身份、关系、任务历史或策略信号的隐私保护证明和声誉后端；
